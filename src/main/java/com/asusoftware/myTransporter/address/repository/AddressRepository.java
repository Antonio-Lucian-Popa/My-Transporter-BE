...
public interface AddressRepository extends JpaRepository<Address, UUID> {
    ...
    @Query("SELECT a.country FROM Address a ORDER BY a.country ASC")
    List<String> findAllCountry();

    @Query("SELECT a.county FROM Address a WHERE a.country=?1 ORDER BY a.county ASC")
    List<String> findAllCounty(String countryName);

    @Query("Select a.city from Address a WHERE a.country=?1 ORDER BY a.city ASC")
    List<String> findAllCity(String countyName);

    @Query("SELECT a FROM Address a WHERE a.country=?1 AND a.county=?2 AND a.city=?3")
    List<Address> findAllAddress(String country, String county, String city);
}
